    ...
    activate(_action: ItemAction, entity: Entity) {
        ...
        if (amountRecovered > 0) {
            window.messageLog.addMessage(
                `You consume ths ${this.parent?.name}, and recover ${amountRecovered} HP!`,
...
export class LightningConsumable extends Consumable {
    ...
    activate(_action: ItemAction, entity: Entity, gameMap: GameMap) {
        ...
        for (const actor of gameMap.actors) {
            if (!Object.is(actor, entity) && gameMap.tiles[actor.y][actor.x].visible) {
...
export class ConfusionConsumable extends Consumable {
    ...
    getAction(): Action | null {
        window.messageLog.addMessage('Select a target location.', Colors.NeedsTarget);
    ...
    activate(action: ItemAction, entity: Entity, gameMap: GameMap) {
        ...
        window.messageLog.addMessage(`The eyes of the ${target.name} look vacant, as it starts to stumble around!`, Colors.StatusEffectApplied);
...
export class FireballDamageConsumable extends Consumable {
    ...
    activate(action: ItemAction, _entity: Entity, gameMap: GameMap): void {
        ...
        for (let actor of gameMap.actors) {
            if (actor.distance(x, y) <= this.radius) {
                ...
                window.messageLog.addMessage(`The ${actor.name} is engulfed in a fiery explosion, taking ${this.damage} damage!`);
