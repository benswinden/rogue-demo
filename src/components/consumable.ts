...
export class ConfusionConsumable extends Consumable {
    ...
    getAction(): Action | null {
        window.engine.messageLog.addMessage('Select a target location.', Colors.NeedsTarget);
    ...
    activate(action: ItemAction, entity: Entity) {
        ...
        window.engine.messageLog.addMessage(`The eyes of the ${target.name} look vacant, as it starts to stumble around!`, Colors.StatusEffectApplied);
    ...
    activate(action: ItemAction, entity: Entity): void {
        ...
        for (let actor of window.engine.gameMap.actors) {
            if (actor.distance(x, y) <= this.radius) {
                window.engine.messageLog.addMessage(`The ${actor.name} is engulfed in a fiery explosion, taking ${this.damage} damage!`);
